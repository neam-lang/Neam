...
// ─── v1.6 NeamMesh — Agentic Process Automation records ──────────────
// All five v1.6 records share the minimal shape used by sub-type 25-34:
// the parsed body lives in fields_json; runtime parses sub-keys on demand.
// Process / Task instances live separately in process_runtime (multi-instance
// per ADR-004) — these registry entries are *declarations* (one per name).
...
struct TaskRecord {
  ...
  std::string agent_ref;          // pre-extracted for fast resolution at runtime
};
...
struct EventRecord {
  std::string name;
  std::string fields_json;        // type: start|intermediate|end|timer|signal|message
  std::string event_type;         // pre-extracted
};

struct PoolRecord {
  std::string name;
  std::string fields_json;        // lanes: { ... }
};
...
class HarnessRegistry {
 public:
  ...
  // v1.6 NeamMesh
  void register_process(ProcessRecord rec);
  void register_task(TaskRecord rec);
  void register_decision(DecisionRecord rec);
  void register_event(EventRecord rec);
  void register_pool(PoolRecord rec);
  ...
  // v1.6 NeamMesh
  const ProcessRecord*          lookup_process(const std::string& name) const;
  const TaskRecord*             lookup_task(const std::string& name) const;
  const DecisionRecord*         lookup_decision(const std::string& name) const;
  const EventRecord*            lookup_event(const std::string& name) const;
  const PoolRecord*             lookup_pool(const std::string& name) const;
  std::vector<std::string>      list_processes() const;
 ...
 private:
  ...
  std::unordered_map<std::string, EventRecord>            events_;
  std::unordered_map<std::string, PoolRecord>             pools_;
};
