...
// v1.6 NeamMesh — additive bridge so process_runtime can execute a single
// task slot through the same LLM + dry-run + provider plumbing the harness
// loop uses, without standing up a full HarnessRecord.  Pure passthrough:
// honours NEAM_HARNESS_DRY_RUN and provider env keys (OPENAI_API_KEY etc.).
