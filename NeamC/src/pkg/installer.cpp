...
bool Installer::verify_checksum(const std::filesystem::path& file, const std::string& expected)
{
  ...
  // SHA-256 implementation using system utilities
  std::string command;
#if defined(__APPLE__)
  command = "shasum -a 256 \"" + file.string() + "\" | cut -d' ' -f1";
#elif defined(_WIN32)
  command = "certutil -hashfile \"" + file.string() + "\" SHA256 | findstr /v \"hash\"";
#else
  command = "sha256sum \"" + file.string() + "\" | cut -d' ' -f1";
#endif
  ...
  std::string computed_lower = computed;
