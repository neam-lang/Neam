...
// ─── v1.6 NeamMesh registration ──────────────────────────────────────
void HarnessRegistry::register_process(ProcessRecord rec) {
  std::lock_guard<std::mutex> lk(mu_);
  processes_[rec.name] = std::move(rec);
}
void HarnessRegistry::register_task(TaskRecord rec) {
  std::lock_guard<std::mutex> lk(mu_);
  tasks_[rec.name] = std::move(rec);
}
void HarnessRegistry::register_decision(DecisionRecord rec) {
  std::lock_guard<std::mutex> lk(mu_);
  decisions_[rec.name] = std::move(rec);
}
void HarnessRegistry::register_event(EventRecord rec) {
  std::lock_guard<std::mutex> lk(mu_);
  events_[rec.name] = std::move(rec);
}
void HarnessRegistry::register_pool(PoolRecord rec) {
  std::lock_guard<std::mutex> lk(mu_);
  pools_[rec.name] = std::move(rec);
}
...
// ─── v1.6 NeamMesh lookups ───────────────────────────────────────────
const ProcessRecord* HarnessRegistry::lookup_process(const std::string& name) const {
  std::lock_guard<std::mutex> lk(mu_);
  auto it = processes_.find(name);
  return it == processes_.end() ? nullptr : &it->second;
}
const TaskRecord* HarnessRegistry::lookup_task(const std::string& name) const {
  std::lock_guard<std::mutex> lk(mu_);
  auto it = tasks_.find(name);
  return it == tasks_.end() ? nullptr : &it->second;
}
const DecisionRecord* HarnessRegistry::lookup_decision(const std::string& name) const {
  std::lock_guard<std::mutex> lk(mu_);
  auto it = decisions_.find(name);
  return it == decisions_.end() ? nullptr : &it->second;
}
const EventRecord* HarnessRegistry::lookup_event(const std::string& name) const {
  std::lock_guard<std::mutex> lk(mu_);
  auto it = events_.find(name);
  return it == events_.end() ? nullptr : &it->second;
}
const PoolRecord* HarnessRegistry::lookup_pool(const std::string& name) const {
  std::lock_guard<std::mutex> lk(mu_);
  auto it = pools_.find(name);
  return it == pools_.end() ? nullptr : &it->second;
}
std::vector<std::string> HarnessRegistry::list_processes() const {
  std::lock_guard<std::mutex> lk(mu_);
  std::vector<std::string> out;
  out.reserve(processes_.size());
  for (const auto& [n, _] : processes_) out.push_back(n);
  return out;
}
