...
//
// Invokes Gemini models via Vertex AI REST API.
// Endpoint: {region}-aiplatform.googleapis.com
// Path: /v1/projects/{project}/locations/{region}/publishers/google/models/{model}:generateContent
// Auth: Bearer token from Application Default Credentials
//
...
namespace
{
std::string build_vertex_url(const std::string& project, const std::string& region,
                              const std::string& model, bool streaming = false)
{
  std::string url = "https://" + region + "-aiplatform.googleapis.com/v1/projects/" + project +
...
nlohmann::json messages_to_vertex_json(const std::vector<Message>& messages,
                                        std::string& system_prompt_out)
{
  ...
  for (const auto& message : messages)
  {
    ...
    nlohmann::json msg;
    // Vertex/Gemini uses "model" instead of "assistant"
...
nlohmann::json tools_to_vertex_json(const std::vector<ToolDefinition>& tools)
{
...
ChatResult extract_vertex_result(const nlohmann::json& json)
{
  ChatResult result;
  if (!json.contains("candidates") || !json.at("candidates").is_array() ||
      json.at("candidates").empty())
  ...
  for (const auto& part : candidate.at("content").at("parts"))
  {
    ...
    {
      ToolCall call;
      call.id = "vertex_call_" + std::to_string(result.tool_calls.size());
  ...
  if (json.contains("usageMetadata"))
  {
    ...
    result.usage.completion_tokens =
        json["usageMetadata"].value("candidatesTokenCount", std::size_t{0});
...
class VertexAdapter final : public LLMProvider
{
public:
  explicit VertexAdapter(ProviderConfig config)
      : config_(std::move(config))
  {
    if (config_.model.empty())
    {
      throw std::runtime_error("Vertex AI model is required");
    }
    project_ = util::get_gcp_project_id();
    if (project_.empty())
    {
      throw std::runtime_error("GCP project ID not found. Set GOOGLE_CLOUD_PROJECT or configure ADC.");
    }
    // Default region
    ...
    {
      // endpoint used as region override
      region_ = config_.endpoint;
    }
    access_token_ = util::get_gcp_access_token();
  }
  ...
  std::string chat(const std::vector<Message>& messages) override
  {
    ...
    payload["contents"] = messages_to_vertex_json(messages, system_prompt);
    ...
    const std::string url = build_vertex_url(project_, region_, config_.model);
    const std::string response = http_post_json(
        url, payload.dump(),
        {"Content-Type: application/json",
         "Authorization: Bearer " + access_token_});
    return extract_vertex_text(nlohmann::json::parse(response));
  }
                             ...
                             const std::string& response_format = "") override
  {
    ...
    if (!tools.empty())
    {
      payload["tools"] = tools_to_vertex_json(tools);
    }
    ...
    const std::string response = http_post_json(
        ...
        {"Content-Type: application/json",
         "Authorization: Bearer " + access_token_});
    return extract_vertex_result(nlohmann::json::parse(response));
  }
                          ...
                          const TokenCallback& token_callback) override
  {
    ...
    const std::string url = build_vertex_url(project_, region_, config_.model, true);
...
{
  return std::make_unique<VertexAdapter>(config);
}
}  // namespace neamc::llm
