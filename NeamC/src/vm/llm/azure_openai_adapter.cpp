...
//
// Reuses OpenAI chat completions format with Azure-specific endpoint and auth.
...
namespace
{
std::string build_azure_url(const std::string& endpoint, const std::string& deployment,
                            const std::string& api_version = "2024-10-21")
{
  ...
  url += "/openai/deployments/" + deployment + "/chat/completions?api-version=" + api_version;
  return url;
}
...
std::string extract_response_text(const nlohmann::json& json)
{
  ...
  throw std::runtime_error("Azure OpenAI response missing content");
}
...
ChatResult extract_chat_result(const nlohmann::json& json)
{
  ...
  if (!json.contains("choices") || !json.at("choices").is_array() ||
      json.at("choices").empty())
  {
    throw std::runtime_error("Azure OpenAI response missing choices");
  }
  const auto& choice = json.at("choices").front();
  if (!choice.contains("message"))
  {
    throw std::runtime_error("Azure OpenAI response missing message");
  }
...
class AzureOpenAIAdapter final : public LLMProvider
{
public:
  explicit AzureOpenAIAdapter(ProviderConfig config)
      : config_(std::move(config))
  {
    if (config_.model.empty())
    {
      throw std::runtime_error("Azure OpenAI deployment name is required");
    }
    if (config_.endpoint.empty())
    {
      throw std::runtime_error("Azure OpenAI endpoint is required");
    }
    if (config_.api_key.empty())
    {
      throw std::runtime_error("Azure OpenAI API key is required");
    }
  }
  ...
  std::string chat(const std::vector<Message>& messages) override
  {
    ...
    const std::string url = build_azure_url(config_.endpoint, config_.model);
...
{
  return std::make_unique<AzureOpenAIAdapter>(config);
}
}  // namespace neamc::llm
