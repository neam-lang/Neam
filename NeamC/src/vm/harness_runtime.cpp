...
// ─── v1.6 NeamMesh — single-step execution bridge ───────────────────
// Pure passthrough to call_llm_or_dry — the same provider/dry-run path the
// harness loop uses.  No registry mutation, no trace emission; the caller
// (process_runtime) owns its own trace plane.
ExecuteStepResult execute_one_step(const ExecuteStepArgs& args)
{
  RunContext tmp;
  tmp.harness_name = args.agent_name;
  tmp.provider     = args.provider;
  tmp.model        = args.model;
  auto pair = call_llm_or_dry(tmp, args.slot, args.agent_name, args.goal, args.prior);
  ExecuteStepResult r;
  r.output = pair.first;
  r.error  = pair.second;
  return r;
