...
void usage()
{
  std::cout << "Usage: neamc <input.neam> [-o output.neamb] [--manifest <json>] [--lsp] [--watch]\n";
...
void deploy_docker(const neamc::DeployConfig& config, const std::string& image_name, bool dry_run)
{
  std::cout << "Deploying to Docker...\n";
  if (!std::filesystem::exists("deploy/docker/Dockerfile"))
  {
    neamc::generate_dockerfile(config);
  }
...
void deploy_kubernetes(const neamc::ProjectManifest& manifest, const std::string& image_name,
                       bool dry_run)
{
  std::cout << "Deploying to Kubernetes...\n";
...
void deploy_lambda(const neamc::ProjectManifest& manifest, bool dry_run)
{
...
int cmd_deploy(int argc, char** argv)
{
  ...
  {
    deploy_docker(manifest.deploy, image_name, dry_run);
  }
  ...
  {
    deploy_lambda(manifest, dry_run);
  }
  else if (target == "cloudrun")
  {
    deploy_cloudrun(manifest, dry_run);
  }
  else if (target == "azure-container-apps")
  {
    std::cout << "Deploying to Azure Container Apps...\n";
    neamc::generate_azure_container_apps(manifest, ".", dry_run);
  }
  else if (target == "azure-aks")
  {
    std::cout << "Deploying to Azure AKS...\n";
    neamc::generate_azure_aks(manifest, ".", dry_run);
  }
  else if (target == "ecs-fargate")
  {
    std::cout << "Deploying to AWS ECS Fargate...\n";
    neamc::generate_ecs_fargate(manifest, ".", dry_run);
  }
  else
  {
    std::cerr << "Unknown deploy target: " << target << "\n";
    std::cerr << "Available targets: local, docker, kubernetes, lambda, cloudrun, azure-container-apps, azure-aks, ecs-fargate\n";
    return 1;
  }
...
hello = "echo Hello from Neam"
  ...
  std::cout << "Created new project: " << name << "\n";
  return 0;
}
...
int compile_once(const std::string& input, const std::string& output, const std::string& manifest)
{
...
int watch_and_compile(const std::string& input, const std::string& output,
                      const std::string& manifest)
{
  ...
  std::cout << "[watch] Watching " << input << " for changes...\n";
  std::cout << "[watch] Press Ctrl+C to stop.\n\n";

  // Initial compile
  std::cout << "[watch] Initial compilation...\n";
  int result = compile_once(input, output, manifest);
  if (result == 0)
  {
    std::cout << "[watch] Compiled successfully: " << output << "\n";
  }

  std::error_code ec;
  auto last_write = fs::last_write_time(input, ec);
  if (ec)
  {
    std::cerr << "[watch] Failed to get file modification time\n";
    return 1;
  }
  ...
  while (true)
  {
    ...
    auto current_write = fs::last_write_time(input, ec);
    if (ec)
    {
      continue;  // File may be temporarily unavailable during save
    }
    ...
    if (current_write != last_write)
    {
      ...
      result = compile_once(input, output, manifest);
      if (result == 0)
      {
        std::cout << "[watch] Compiled successfully: " << output << "\n";
      }
...
int compile_input(const std::vector<std::string>& args)
{
  ...
  if (watch)
  {
    return watch_and_compile(input, output, manifest);
  }
