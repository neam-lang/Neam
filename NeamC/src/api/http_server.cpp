...
private:
  ...
  HttpResponse handle_request(const HttpRequest& request)
  {
    ...
    // Find route handler - try exact match first
...
void HttpServer::get_prefix(const std::string& prefix, RouteHandler handler)
{
  impl_->get_prefix(prefix, std::move(handler));
}
